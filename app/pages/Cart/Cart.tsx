...
const CartPage: React.FC = () => {
  ...
  const removeItem = async (componentId: string) => {
    ...
    try {
      await axios.delete(
        "https://cnerlab.onrender.com/api/v1/cart/user/component",
        {
          data: { componentId },
          withCredentials: true,
        }
  ...
  const clearCart = async () => {
    setUpdating("clear");
    try {
      await axios.delete("https://cnerlab.onrender.com/api/v1/cart/user", {
        withCredentials: true,
      });
      setCartItems([]);
  ...
  const requestPermission = async () => {
    ...
    try {
      for (const item of cartItems) {
        try {
          await axios.post(
            "https://cnerlab.onrender.com/api/v1/permission/user/add",
            {
